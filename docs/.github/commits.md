...
## Commit 45: Source Scoping Architecture, Conversational Chat Redesign, SQLite State Persistence, and Discovery Controls
- **Granular Source Scoping & Dossier Inspector (`SourceCard.jsx`, `SourceList.jsx`, `SourceInspector.jsx`, `Sidebar.jsx`, `PromptBar.jsx`)**: Added interactive scoping checkboxes per card and a master "Select All" toggle header to selectively isolate sources for grounded chat synthesis. Extracted a modular `SourceInspector.jsx` dossier drawer featuring live reading time, word count metrics, and quick actions ("Explain Source", "Chat Only", and markdown copy), with real-time ratio badges on the prompt bar (`3/5 sources`).
- **Selection State Persistence Across Refresh (`useSources.js`, `NotebookPage.jsx`)**: Connected `notebookId` to `useSources` with reactive `localStorage` synchronization (`sourcebook_scoped_sources_${notebookId}`). Ingested sources default to 100% selected on initial load, and custom selections, isolations, and exclusions are strictly preserved across browser reloads.
- **SQLite Settings Persistence & Server Bootstrapping (`db.go`, `settings_repo.go`, `api.go`)**: Added automatic schema migration for the `user_settings` table to store LLM provider, base URL, model, and local API key preferences. The server automatically restores saved configurations into the active LLM client on boot, preserving 9router and local proxy setups without leaking credentials into code or git.
- **LLM Health Caching & Proxy Optimization (`health_handler.go`, `models_handler.go`)**: Expanded connection test timeouts to 10 seconds and introduced a thread-safe 30-second TTL health cache, slashing polling latency from ~6.5s to sub-millisecond and preventing proxy request flooding.
- **Conversational Chat Bubble Overhaul (`MessageBubble.jsx`, `ThinkingIndicator.jsx`, `SourceDetailsPanel.jsx`, `ChatStudio.jsx`)**: Replaced bulky card boxes with sleek, right-aligned user chat bubbles (`user-chat-bubble`) and an open editorial layout for assistant responses. Converted boxed thinking frames into inline pulsing indicators, condensed consulted sources into expandable pill chips, replaced `<select>` inputs with segmented buttons (`[Sources] [Web] [Hybrid]`), and resolved a runtime `Globe` icon import dependency.
- **Enhanced Web Discovery Multi-Select & Filtering (`SourceDiscovery.jsx`, `index.css`)**: Built a robust web import panel with "Select all (X/Y)" controls, type filter pills (`All`, `Web`, `YouTube`), distinct result checkboxes, and a bottom action bar with live selection counters.
- **Strict Modularity Adherence**: Refactored `SourceDiscovery.jsx` (177 lines), `useSources.js` (182 lines), `SourceList.jsx` (117 lines), and all associated files to ensure 100% compliance with the strict sub-200-line architectural ceiling.

## Commit 46: Universal SQLite Settings Control, Grounded RAG Resilience, UI Ergonomics, and Developer Tooling
- **Universal Endpoint Persistence in SQLite (`db.go`, `settings_repo.go`, `settings.go`, `main.go`)**: Extended the SQLite `user_settings` schema with dedicated columns for all service endpoints (`searxng_url`, `searqon_url`, `youtube_service_url`, `embedding_provider`, `embedding_url`, `embedding_model`). Auto-migrated schema and wired dynamic controller resolution with robust fallback defaults, allowing full endpoint configuration directly from the frontend UI without editing environment files or restarting containers.
- **Modular Settings Subsystem (`SettingsPage.jsx`, `LLMSettingsTab.jsx`, `SearchSettingsTab.jsx`, `MediaSettingsTab.jsx`, `EmbeddingSettingsTab.jsx`)**: Decomposed the monolithic settings page into four focused tabs, keeping every file strictly under 200 lines (`LLMSettingsTab`: 143 lines, `MediaSettingsTab`: 82 lines, `SearchSettingsTab`: 61 lines, `EmbeddingSettingsTab`: 60 lines, `SettingsPage`: 190 lines). Preserved API keys across updates and added real-time status feedback.
- **Fast Lightweight LLM Health Probes (`client.go`, `health_handler.go`, `models_handler.go`)**: Replaced slow, blocking token-generation test pings in the health handler with a non-blocking `Ping()` probe that checks `/models` (OpenAI-compatible) and `/api/tags` (Ollama) in under 500ms. Added instant health cache invalidation on configuration changes and differentiated online (30s) vs offline (3s) cache durations to prevent stale "LLM Offline" status badges.
- **Embedding Circuit Breaker & 0ms Fallback (`vector/client.go`)**: Introduced a 2-second connection timeout and an automatic 30-second cooling circuit breaker for external embedding endpoints. When nomic or Ollama is offline, calls immediately fail over to BM25 lexical reranking in 0ms, eliminating 60+ second request hangs and deadlocks.
- **Fix for Comparative Synthesis Bug (`chat/explain.go`)**: Eliminated destructive query-filtering LLM prompts in `BatchAndSummarize` that caused `/compare` queries to fail with "No sources provided to compare (only a title snippet)". Content is now sanitized, normalized, and bounded preserving full source fidelity for grounded multi-document synthesis.
- **100% Emoji Removal & Button Restyling (`MessageBubble.jsx`, `ChatStudio.jsx`, `SourceDiscovery.jsx`, `index.css`)**: Removed all emojis across the entire UI codebase in favor of crisp typography and Lucide icons. Restyled the "Select all / Clear all" and "Import" buttons with clean, modern styling, subtle borders, and smooth hover interactions.
- **Frontend Error Boundaries & Resilience (`ErrorBoundary.jsx`, `NotebookPage.jsx`)**: Implemented a localized React Error Boundary wrapping workspace panels (Sources, ChatStudio, NotesStudio) to gracefully isolate unexpected runtime component crashes and offer a one-click "Reload Panel" recovery mechanism.
- **Global Drag-and-Drop Ingestion (`NotebookPage.jsx`)**: Added workspace-wide file drag-and-drop support with a full-screen visual dropzone overlay, parsing PDFs, Markdown, and plain text files directly into notebook sources via client-side workers.
- **Instant URL Detection & Slash Commands (`PromptBar.jsx`, `ChatStudio.jsx`)**: Enhanced prompt input with automatic URL detection chips ("+ Ingest as Source"), quick slash commands (`/summarize`, `/explain`, `/compare`, `/quiz`), and a prominent "Stop Generating (Esc)" button during active streams.
- **Interactive Follow-Up Chips (`MessageBubble.jsx`)**: Rendered context-aware follow-up suggestion chips ("Summarize key takeaways", "What are the limitations?", "Explain with an example") beneath completed assistant responses to drive fluid research exploration.
- **Auto-Dismissing Health Alert Banner (`ChatStudio.jsx`, `SettingsPage.jsx`)**: Added interactive status indicators with direct navigation to Settings; offline alert banners automatically fade and vanish after 5 seconds with a one-click dismiss option, and connection test notifications auto-clear after 4 seconds.
- **Developer Automation Suite (`Makefile`)**: Added a root `Makefile` supporting one-command builds, server and UI dev modes, unit tests, code line count verification (`make lines`), and container management.
- **Production Static Asset Serving (`static_handler.go`, `main.go`)**: Wired automatic SPA static asset serving from `ui/dist` with client-side fallback into the Go server, enabling single-binary execution on port 5000 without requiring a separate Node.js dev server.
- **Container Infrastructure & Dev Setup (`docker-compose.yml`, `Dockerfile`, `.env.example`, `README.md`)**: Configured reproducible multi-container orchestration for the Go server, SearXNG privacy search, and Python YouTube transcript services alongside clean environment templates.
- **Database Restoration**: Repaired and rebuilt SQLite database indices, verifying complete recovery of all 25 notebooks and 228 sources with passing PRAGMA integrity checks.

## Commit 47: Production Stack Orchestration, Isolated Per-Provider AI Configuration, Foreign Key Cascade, and Settings Architecture
- **Production-Grade Microservice Orchestration (`Makefile`)**: Implemented the unified `make run` production target that compiles the React SPA frontend into `ui/dist`, builds the Go binary `bin/sourcebook-server`, clears stale ports, and supervises all microservices (Searqon: 4001, Document: 4002, Jina: 4003, Reddit: 4004, Social: 4005, YouTube: 6001, and SourceBook: 5000) under a single process tree with signal trapping. Added `make stop` and IPv4-resilient `make status` targets.
- **Isolated Per-Provider AI Configuration (`LLMSettingsTab.jsx`, `SettingsPage.jsx`)**: Completely decoupled state management across LLM provider protocols (OpenAI Compatible, Ollama Native, Groq Cloud, NVIDIA NIM). Configured base endpoint URLs, targeted model names, and API keys are now maintained in discrete provider configurations, preventing credential bleeding or field collision when toggling between providers.
- **Dual-Layer Settings Persistence (`db.go`, `settings_repo.go`, `models/settings.go`, `SettingsPage.jsx`)**: Added schema auto-migration for a `provider_configs` JSON column in the SQLite `user_settings` table. Provider settings are stored persistently in the database on save and kept reactively synchronized in `localStorage`, guaranteeing full state retention across browser sessions and server restarts.
- **Modular Settings Architecture (`SettingsSidebar.jsx`, `SettingsHeader.jsx`, `SettingsPage.jsx`)**: Decomposed `SettingsPage.jsx` into modular components (`SettingsSidebar.jsx` and `SettingsHeader.jsx`), reducing `SettingsPage.jsx` to 194 lines and keeping all settings components strictly compliant with the sub-200-line ceiling rule.
- **SQLite Foreign Key Cascades & Sentinel Cleanup (`db.go`, `notebook_repo.go`, `sentinel_handler.go`, `SentinelStatus.jsx`)**: Enabled SQLite foreign key enforcement (`_foreign_keys=on`) and explicit cascading deletions to prevent orphaned source records upon notebook deletion. Scoped Sentinel scraper agent status to the active notebook, preventing phantom scraping indicators on empty notebooks. Modularized background scraper logic into `internal/agent/repair.go` and `internal/agent/sentinel.go` adhering to the sub-200-line ceiling.
- **HTTP Source Ping Resilience (`source_ping_handler.go`)**: Upgraded `/api/sourcebook/v1/sources/ping` with a modern browser User-Agent header and fallback GET range requests, eliminating false-negative offline badges for Wikipedia and redirected endpoints.
- **YouTube Service Stability & Subprocess Fallback (`config.py`, `youtube_transcript.go`, `youtube_handler.go`)**: Added `extra = "ignore"` to Pydantic settings in the YouTube FastAPI microservice to prevent startup crashes from unrecognized environment variables. Added direct Go subprocess execution fallback to invoke Python transcript scrapers natively when port 6001 is unreachable.
- **Searqon Scraper Anti-Bot False Positive Resolution (`bot_detection.go`, `antibot.go`)**: Fixed false-positive bot challenge detections triggered by client-side script matches (e.g. hcaptcha keywords in MediaWiki scripts) by adding status-200 word count validation and modern Chromium personas.
- **Minimalist Branding & UI Discovery Polish (`AppShell.jsx`, `index.css`, `SourceDiscovery.jsx`)**: Removed the icon brand logo container from `AppShell.jsx` and added `.brand-logo { display: none !important; }` in `index.css` to present a clean, text-only "SourceBook" navigation link across non-notebook pages. Cleaned up the web discovery import action button by removing redundant count suffixes in favor of a clean, minimalist label.

## Commit 48: NotebookLM Chat UI Overhaul, Floating Dock, High-Contrast Segmented Controls, and Chat Modularization
- **NotebookLM Signature Floating Dock (`PromptBar.jsx`)**: Rebuilt the input bar into a centered frosted-glass floating dock (`max-width: 780px`) featuring an integrated source scoping badge (`X sources`), seamless expanding multiline input, quick slash command popup (`/summarize`, `/explain`, `/compare`, `/quiz`), circular upward-pointing send button (`ArrowUp`) with dynamic high-contrast active states (`#09090b` arrow on `#f4f4f5` white circular pill), and centered verification disclaimer.
- **Header Collision & Text Overlap Elimination (`ChatHeader.jsx`)**: Replaced the wrapping multi-row header with a non-wrapping single-row flex container (`height: 48px`, `whiteSpace: nowrap`, `overflow: hidden`, `flexShrink: 0`). Removed redundant full-width panel toggle buttons that occupied excessive header space when sidebars were visible, displaying only compact icon toggles (`PanelLeftOpen`, `PanelRightOpen`) when a sidebar is collapsed, eliminating content collision with the chat viewport.
- **Segmented Mode Switcher High-Contrast Resolution (`ChatHeader.jsx`)**: Resolved an invisible white-on-white text contrast defect on the active segmented pill (`Sources`, `Web`, `Hybrid`) by replacing generic `--accent-primary` variables with explicit high-contrast tokens (`#09090b` jet-black label and icon over a `#f4f4f5` solid white pill with `0 1px 3px rgba(0, 0, 0, 0.3)` drop shadow), ensuring instant legibility across all dark workspace themes.
- **NotebookLM Guide Card & Floating Prompt Chips (`ChatWelcomeScreen.jsx`)**: Replaced the 4 full-width blocky grey bars with a NotebookLM guide card featuring workspace title, source count indicator, and an array of horizontal floating pill prompt chips ("Summarize Sources", "FAQ & Concepts", "Study Guide", "Compare Perspectives", "Briefing Doc", "Table of Contents") with smooth hover animations.
- **Editorial Centered Reading Column (`ChatStudio.jsx`, `MessageBubble.jsx`)**: Centered conversation messages in an airy 780px column. Assistant responses render open-canvas markdown without clunky card borders, featuring glowing sparkle avatars, inline citation pills with hovercard source excerpts, consulted source pills, copy/save note toolbars, and dynamic follow-up suggestion chips.
- **Chat Subsystem Modularization (`ChatStudio.jsx`, `ChatHeader.jsx`, `ChatWelcomeScreen.jsx`, `PromptBar.jsx`, `MessageBubble.jsx`, `SourceDetailsPanel.jsx`, `CitationPill.jsx`, `ThinkingIndicator.jsx`, `ChatShortcutsModal.jsx`, `ChatControls.jsx`)**: Decomposed monolithic 578-line `ChatStudio.jsx` into 10 focused sub-200-line modular components, ensuring 100% compliance with the strict `<200 lines` architectural rule.
